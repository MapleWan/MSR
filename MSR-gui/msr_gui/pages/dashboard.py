...
async def dashboard_page():
    ...
    with create_layout('仪表盘'):
        ui.label('仪表盘').classes('text-2xl font-bold text-stone-800 q-mb-md')
            ...
            with ui.row().classes('items-center justify-between p-5'):
                with ui.row().classes('items-center gap-4'):
                    ...
                    with ui.column().classes('gap-1'):
                        ui.label('仓库路径').classes('text-xs uppercase tracking-wide text-stone-400')
                        ui.label(status.get('path', '未知')).classes('text-base text-stone-800 font-medium')
        ...
        with ui.row().classes('w-full gap-5 q-mb-md'):
            with ui.card().classes('col msr-card'):
                with ui.column().classes('p-5 gap-3'):
                    ui.label('Rules').classes('text-xs uppercase tracking-wide text-stone-400')
                    ui.label(str(rules_count)).classes('text-3xl font-bold text-slate-600')
                    ui.icon('rule').classes('text-slate-400').style('font-size: 20px;')
            ...
            with ui.card().classes('col msr-card'):
                with ui.column().classes('p-5 gap-3'):
                    ui.label('MCP').classes('text-xs uppercase tracking-wide text-stone-400')
                    ui.label(str(mcp_count)).classes('text-3xl font-bold text-slate-500')
                    ui.icon('hub').classes('text-slate-400').style('font-size: 20px;')

        # ---- 已支持 IDE 列表 ----
        with ui.card().classes('w-full q-mb-md msr-card'):
            with ui.row().classes('items-center p-5 border-b border-stone-100 gap-2'):
                ui.icon('computer', size='22px').classes('text-slate-600')
                ui.label('已支持 IDE').classes('text-lg font-semibold text-stone-700')
            with ui.row().classes('gap-3 p-5 wrap'):
                for ide in ide_list:
                    name = ide.get('name', 'Unknown')
                    icon_url = get_ide_icon_url(name)
                    with ui.card().classes('msr-card-hover cursor-pointer').style('border: 1px solid #E8E4E0;'):
                        with ui.row().classes('items-center px-4 py-2 gap-2'):
                            if icon_url:
                                ...
                                ui.icon('computer').classes('text-slate-600')
                            ui.label(name.capitalize()).classes('text-sm text-stone-700 font-medium')

        # ---- 快捷操作区 ----
        with ui.card().classes('w-full msr-card'):
            with ui.row().classes('items-center p-5 border-b border-stone-100 gap-2'):
                ui.icon('bolt', size='22px').classes('text-[#C4A882]')
                ui.label('快捷操作').classes('text-lg font-semibold text-stone-700')
