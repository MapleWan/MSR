...
- **压缩包名称推断：** 当压缩包内文件直接散落在根目录（无顶层子目录包裹）时，使用压缩包文件名（去除扩展名）作为配置名称
...
- **fix**: 修复压缩包导入时，当文件直接散落在压缩包根目录（无顶层子目录包裹）时，skill/mcp 名称会变为临时目录随机名的问题，现在正确使用压缩包文件名作为名称

### 0.1.2

- 首次公开发布
- 支持 7 款 AI IDE（Trae、Qoder、Lingma、CodeBuddy、Cursor、Kiro、Antigravity）
- 支持 rules、skills、MCP 三种配置类型的导入、同步、查看、删除
- 支持多版本管理
- 支持全局配置文件
