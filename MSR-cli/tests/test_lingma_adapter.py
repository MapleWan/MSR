...
验证需求: 5.2, 11.6, 11.7, 11.8, 11.9, 11.10
"""
...
class TestLingmaAdapterProperties:
    """测试 Lingma 适配器基本属性"""

    def test_ide_name(self, adapter):
        """ide_name 应返回 'lingma'"""
        assert adapter.ide_name == "lingma"
...
class TestLingmaRulesPath:
    """测试 Lingma rules 路径解析 (需求 11.6)"""
    ...
    def test_project_scope(self, adapter):
        ...
        assert path == project / ".lingma" / "rules" / "coding-style.md"
    ...
    def test_global_scope_returns_path(self, adapter):
        ...
        assert ".lingma" in str(path)
        assert "rules" in str(path)


class TestLingmaSkillsPath:
    """测试 Lingma skills 路径解析 (需求 11.7, 11.8)"""

    def test_project_scope(self, adapter):
        """项目级 skills 路径: <project>/.lingma/skills/<name>/"""
        project = Path("/my/project")
        path = adapter.get_skills_path("my-skill", "project", project)
        assert path == project / ".lingma" / "skills" / "my-skill"

    def test_global_scope(self, adapter):
        """用户级 skills 路径: ~/.lingma/skills/<name>/"""
        path = adapter.get_skills_path("my-skill", "global")
        assert path == Path.home() / ".lingma" / "skills" / "my-skill"


class TestLingmaMcpPath:
    """测试 Lingma MCP 路径解析 (需求 11.9, 11.10)"""

    @patch("msr_sync.adapters.lingma.PlatformInfo.get_os", return_value="macos")
    @patch(
        "msr_sync.adapters.lingma.PlatformInfo.get_app_support_dir",
        return_value=Path.home() / "Library" / "Application Support",
    )
    def test_macos_mcp_path(self, mock_app_dir, mock_os, adapter):
        """macOS MCP 路径: ~/Library/Application Support/Lingma/SharedClientCache/mcp.json"""
    ...
    @patch("msr_sync.adapters.lingma.PlatformInfo.get_os", return_value="windows")
    @patch(
        "msr_sync.adapters.lingma.PlatformInfo.get_app_support_dir",
        return_value=Path.home() / "AppData" / "Roaming",
    )
    def test_windows_mcp_path(self, mock_app_dir, mock_os, adapter):
        """Windows MCP 路径: %APPDATA%/Lingma/SharedClientCache/mcp.json"""
        path = adapter.get_mcp_path()
        expected = (
            Path.home()
            / "AppData"
...
class TestLingmaFormatRuleContent:
    """测试 Lingma format_rule_content (需求 5.2)"""

    def test_adds_lingma_header(self, adapter):
        """应添加 Lingma frontmatter 头部"""
        content = "# My Rule\nSome content here."
...
class TestLingmaScanExistingConfigs:
    """测试 Lingma scan_existing_configs"""

    def test_returns_correct_structure(self, adapter, tmp_path):
        """返回值应包含 rules、skills、mcp 三个键"""
        with patch(
            "msr_sync.adapters.lingma.PlatformInfo.get_home",
    ...
    def test_rules_always_empty(self, adapter, tmp_path):
        """rules 列表始终为空（Lingma 不支持全局 rules）"""
        with patch(
            "msr_sync.adapters.lingma.PlatformInfo.get_home",
            return_value=tmp_path,
        ):
    ...
    def test_scans_user_skills(self, adapter, tmp_path):
        """应扫描 ~/.lingma/skills/ 下的子目录"""
        skills_dir = tmp_path / ".lingma" / "skills"
    ...
    def test_scans_mcp_file(self, adapter, tmp_path):
        """应检测 MCP 配置文件是否存在"""
        mcp_dir = tmp_path / "Lingma" / "SharedClientCache"
