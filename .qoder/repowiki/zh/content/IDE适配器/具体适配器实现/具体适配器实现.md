...
## 简介
本文件聚焦于 MSR 项目中具体 IDE 适配器的实现细节，深入分析 CodeBuddy、Qoder、Lingma、Trae、Kiro、Antigravity 六个适配器的独特特性，涵盖路径解析策略、配置格式转换规则、差异化处理机制、配置扫描实现以及平台兼容性与跨平台路径处理策略。特别强调 CodeBuddy、Kiro 支持全局 rules 的关键差异，以及各适配器在 MCP 配置合并上的不同处理方式。

**更新** 新增 Kiro（AWS）和 Antigravity（Google）适配器，将适配器生态系统从4个平台扩展到7个平台
...
**更新** 架构图现已包含 Kiro 和 Antigravity 适配器，扩展适配器生态系统支持
...
- [frontmatter.py:10-177](file://MSR-cli/msr_sync/core/frontmatter.py#L10-L177)
...
- 具体适配器：CodeBuddy、Qoder、Lingma、Trae、Kiro、Antigravity 分别实现各自的路径解析、格式转换与扫描逻辑
- 注册表 Registry：管理 IDE 名称到适配器类的映射，支持延迟加载与实例缓存
- 平台检测 PlatformInfo：提供跨平台路径解析（主目录、应用数据目录）
- frontmatter 模块：提供统一的 frontmatter 解析与各 IDE 模板头部生成，包括 Antigravity 专用头部
- 命令层：init 与 sync 命令分别负责配置扫描导入与同步落地

**更新** 新增 Kiro 和 Antigravity 适配器，扩展支持 AWS 和 Google 生态系统
...
**更新** 架构图现已包含 Kiro 和 Antigravity 适配器，扩展支持矩阵

图表来源
- [registry.py:48-91](file://MSR-cli/msr_sync/adapters/registry.py#L48-L91)
...
- [frontmatter.py:110-177](file://MSR-cli/msr_sync/core/frontmatter.py#L110-L177)
- [platform.py:9-60](file://MSR-cli/msr_sync/core/platform.py#L9-L60)
...
**更新** CodeBuddy 适配器保持不变，仍是唯一支持全局 rules 的 IDE
...
### Kiro 适配器（新增）
- 路径解析策略
  - 支持项目级与用户级 rules（steering）和 skills
  - 项目级 rules 路径位于 <project>/.kiro/steering/<name>.md
  - 用户级 rules 路径位于 ~/.kiro/steering/<name>.md
  - 项目级 skills 路径位于 <project>/.kiro/skills/<name>/
  - 用户级 skills 路径位于 ~/.kiro/skills/<name>/
  - MCP 路径固定为 ~/.kiro/mcp.json（跨平台一致）
- 格式转换规则
  - 不添加额外头部，直接返回原始内容
- 配置扫描实现
  - 扫描用户级 rules（steering）、skills、MCP 文件，返回结构化的配置清单
- 差异化处理机制
  - supports_global_rules() 返回 True，支持全局级同步
  - 与 CodeBuddy 类似，但使用 steering 作为规则目录而非 rules
- 平台兼容性
  - 使用 PlatformInfo.get_home() 获取用户主目录，MCP 路径在 macOS 与 Windows 相同
...
**新增** Kiro 适配器支持 AWS 生态系统，与 CodeBuddy 类似但使用不同的目录结构
...
- [kiro.py:21-133](file://MSR-cli/msr_sync/adapters/kiro.py#L21-L133)
- [platform.py:32-60](file://MSR-cli/msr_sync/core/platform.py#L32-L60)
- [test_kiro_adapter.py:19-214](file://MSR-cli/tests/test_kiro_adapter.py#L19-L214)

### Antigravity 适配器（新增）
- 路径解析策略
  - 仅支持项目级 rules；当 scope 为 global 时仍返回路径（调用方负责检查 supports_global_rules() 并输出警告）
  - 项目级 rules 路径位于 <project>/.agents/rules/<name>.md
  - 用户级 rules 不支持（Antigravity 全局 rules 为单文件 GEMINI.md，与 MSR 多文件模型不兼容）
  - 项目级 skills (workflows) 路径位于 <project>/.agents/workflows/<name>.md
  - 用户级 skills (workflows) 路径位于 ~/.gemini/workflows/<name>.md
  - MCP 路径位于 ~/.gemini/antigravity/mcp_config.json
- 格式转换规则
  - 不添加额外头部，直接返回原始内容
- 配置扫描实现
  - rules 列表始终为空（不支持全局 rules）
  - 扫描用户级 skills（workflows）与 MCP 文件
- 差异化处理机制
  - supports_global_rules() 返回 False，全局级同步将被跳过并提示
  - skills 对应 Antigravity 的 workflows，存储为 .md 文件
- 平台兼容性
  - 使用 PlatformInfo.get_home() 获取用户主目录，MCP 路径在 macOS 与 Windows 相同
...
**新增** Antigravity 适配器支持 Google 生态系统，使用 .agents 目录结构
...
- [antigravity.py:22-131](file://MSR-cli/msr_sync/adapters/antigravity.py#L22-L131)
- [platform.py:32-60](file://MSR-cli/msr_sync/core/platform.py#L32-L60)
- [test_antigravity_adapter.py:19-195](file://MSR-cli/tests/test_antigravity_adapter.py#L19-L195)
...
- Kiro：扫描用户级 rules（steering，支持全局）、skills、MCP
- Antigravity：rules 列表为空（不支持全局），扫描用户级 skills（workflows）、MCP

**更新** 新增 Kiro 和 Antigravity 的配置扫描实现对比

```mermaid
flowchart TD
Start(["开始扫描"]) --> GetHome["获取用户主目录"]
GetHome --> ScanRules["扫描用户级 rules<br/>CodeBuddy/Kiro 支持全局<br/>Antigravity 不支持全局"]
GetHome --> ScanSkills["扫描用户级 skills<br/>部分 IDE 使用 .trae-cn<br/>Antigravity 使用 workflows"]
...
**更新** 流程图现已包含 Kiro 和 Antigravity 的扫描策略
...
**更新** 同步流程保持不变，但新增了 Kiro 和 Antigravity 的头部处理
...
- 适配器依赖关系
  ...
  - CodeBuddy、Qoder、Lingma、Kiro、Antigravity 依赖 frontmatter 模块生成 IDE 特定头部
  - 五个适配器均依赖 PlatformInfo 进行跨平台路径解析
- 注册表与命令层
  - 注册表负责 IDE 名称到适配器类的延迟加载与实例缓存
  - init 与 sync 命令通过注册表获取适配器实例并调用其方法

**更新** 新增 Kiro 和 Antigravity 适配器的依赖关系
...
**更新** 依赖图现已包含 Kiro 和 Antigravity 适配器
...
- [kiro.py:17-20](file://MSR-cli/msr_sync/adapters/kiro.py#L17-L20)
- [antigravity.py:17-20](file://MSR-cli/msr_sync/adapters/antigravity.py#L17-L20)
- [registry.py:8-91](file://MSR-cli/msr_sync/adapters/registry.py#L8-L91)
...
- 全局级 rules 同步失败
  - 现象：全局级同步被跳过并提示不支持
  - 原因：Qoder、Lingma、Trae、Antigravity 不支持全局 rules
  - 处理：改为项目级同步或仅在 CodeBuddy、Kiro 使用全局级
- MCP 配置合并冲突
...
- Antigravity 兼容性问题
  - 现象：全局 rules 同步无效
  - 原因：Antigravity 使用单文件 GEMINI.md，不支持多文件模型
  - 处理：仅使用项目级 rules 或考虑其他适配器
- Kiro 规则目录混淆
  - 现象：找不到规则文件
  - 原因：Kiro 使用 steering 目录而非 rules 目录
  - 处理：确认规则文件位于 .kiro/steering/ 目录下

**更新** 新增 Kiro 和 Antigravity 的故障排查指南
...
## 结论
- CodeBuddy 和 Kiro 是支持全局 rules 的 IDE 适配器，这使其在统一仓库与多项目场景中具有独特优势
- Qoder、Lingma、Trae、Antigravity 在路径解析与配置扫描上保持一致的"仅支持项目级"策略，MCP 路径随平台变化
- Kiro 与 CodeBuddy 类似，但使用不同的目录结构（steering vs rules）
- Antigravity 使用 .agents 目录结构，skills 对应 workflows，且不支持全局 rules
- 适配器通过统一接口与注册表实现解耦，frontmatter 模块确保规则内容在不同 IDE 间的一致性
- 命令层在同步前进行能力检查与冲突提示，保证用户体验与数据安全

**更新** 结论现已包含 Kiro 和 Antigravity 的特点，扩展了适配器生态系统支持
...
- 配置示例与路径约定（基于适配器实现）
  ...
  - Kiro（新增）
    - 项目级 rules（steering）：项目根目录/.kiro/steering/<name>.md
    - 用户级 rules（steering）：~/.kiro/steering/<name>.md
    - 项目级 skills：项目根目录/.kiro/skills/<name>/
    - 用户级 skills：~/.kiro/skills/<name>/
    - MCP：~/.kiro/mcp.json（跨平台一致）
  - Antigravity（新增）
    - 项目级 rules：项目根目录/.agents/rules/<name>.md
    - 项目级 skills（workflows）：项目根目录/.agents/workflows/<name>.md
    - 用户级 skills（workflows）：~/.gemini/workflows/<name>.md
    - MCP：~/.gemini/antigravity/mcp_config.json（跨平台一致）

**更新** 附录现已包含 Kiro 和 Antigravity 的配置路径约定
...
- [kiro.py:5-12](file://MSR-cli/msr_sync/adapters/kiro.py#L5-L12)
- [antigravity.py:5-12](file://MSR-cli/msr_sync/adapters/antigravity.py#L5-L12)