...
### CLI 与参数绑定
- 参数
  - --ide：可多次指定，支持 trae/qoder/lingma/codebuddy/all
...
### 适配器集成与 --ide 参数
- 适配器注册表支持的 IDE：qoder、lingma、trae、codebuddy、cursor
- --ide=all 展开为所有已注册适配器
- 每个适配器需实现：
...
- 适配器层：
  ...
  - CodeBuddyAdapter/QoderAdapter 实现具体路径与头部格式
...
- 不支持的 IDE：确认 --ide 为 trae/qoder/lingma/codebuddy/all
- 全局级 rules 不支持：Trae、Qoder、Lingma 不支持用户级 rules，使用项目级同步
- 权限不足：检查目标路径写入权限
